...
<html lang="en">
	...
	<body class="app sidebar-mini">
		...
		<main class="app-content">
			<div class="app-title">
				<div>
					<h4><i class="fa fa-dollar fa-lg"></i> MEMBER PAYMENTS</h4>
					<p>Member Payments List Page</p>
				</div>
				<ul class="app-breadcrumb breadcrumb">
					<li class="breadcrumb-item"><i class="fa fa-home"></i></li>
					<li class="breadcrumb-item"><a href="member-payments">Member Payments</a></li>
				</ul>
			</div>
				...
				<div class="col-md-12">
				    <?php
                        if ($_SESSION['userType'] == 0) {
    					    ?><button type="button" class="btn btn-success pull-right" data-toggle="modal" data-target="#registerMemberPaymentModal"><i class="fa fa-plus"></i> New Member Payment</button><?php
                        } else {
                            if ($role_rs[3] == '1') {
    					        ?><button type="button" class="btn btn-success pull-right" data-toggle="modal" data-target="#registerMemberPaymentModal"><i class="fa fa-plus"></i> New Member Payment</button><?php
                            }
					...
					<div class="tile">
						<div class="tile-body">
							<div class="table-responsive">
								<table class="table table-striped table-bordered table-sm" id="memberPaymentsTable">
									<thead>
										<tr align="center">
											<th>Payment ID</th>
											<th>Member Name</th>
												...
												?>
													<tr>
														...
														<?php
                                                            if ($_SESSION['userType'] == 0) {
                                                                ?><td align="center">
																	<span class="btn btn-danger btn-sm btnCancelMemberPayment" id="<?php echo "Cancel". $rs[0]; ?>" data-id="<?php echo $rs[0]. "###0". "###". $_SESSION['userIdd']. "###". date("Y-m-d H:s:i"); ?>"><i class="fa fa-fw fa-lg fa-times"></i> Cancel</span> 
                                                                </td><?php
                                                            } else {
                                                                if ($role_rs[5] == '1') {
                                                                    ?><td align="center">
		...
		<script type="text/javascript">
			$('#memberPaymentsTable').DataTable({"pageLength": 100})
			$("#cmbMemberPaymentsMemberName").select2({ tags: false, dropdownParent: $("#registerMemberPaymentModal") })
			$("#cmbMemberPaymentsAccountName").select2({ tags: true, dropdownParent: $("#registerMemberPaymentModal") })
			$("#cmbInvestPaymentInvestorIDUp").select2({ tags: true, dropdownParent: $("#updateInvestorPaymentModal") })
			$("#cmbInvestPaymentBankNameUp").select2({ tags: true, dropdownParent: $("#updateInvestorPaymentModal") })
		</script>
