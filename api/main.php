...
function userNamesPrivileges($conn)
{
	$user_names = array();
	$data = array();
	$result = mysqli_query($conn, "SELECT users.userID, users.userName, employees.empName FROM users JOIN employees ON (users.empID = employees.empID AND employees.empStatus = 1 AND users.registeredBy = '" . $_SESSION['userIdd'] . "')");
	if ($result) {
		$num_rows = mysqli_num_rows($result);
		if ($num_rows > 0) {
...
function login_process($conn)
{
	...
	if ($checkUser) {
		...
		if ($countUsers > 0) {
			...
			$checkMember = mysqli_query($conn, "SELECT memberID, memberName, memberTelephone, memberEmail, memberPasscode, memberPhoto, memberStatus FROM members WHERE memberTelephone = '" . $user_name . "' OR memberEmail = '" . $user_name . "' OR memberID = '" . $user_name . "'");
			if ($checkMember) {
				if (mysqli_num_rows($checkMember) > 0) {
					$memberDetails = mysqli_fetch_array($checkMember);
					$memmberr_Status = $memberDetails[6];
					if ($memmberr_Status == "0") {
						$errorMsg = "Sorry, <b>" . $user_name . "</b> is bloacked by the admin, please contact the administrator";;
						$errorList[] = $errorMsg;
						$data = json_encode(array('Status' => false, 'Message' => $errorList));
					} else {
						$memberrLoginQry = mysqli_query($conn, "SELECT memberPasscode FROM members WHERE (memberTelephone = '" . $memberDetails[2] . "' OR memberEmail = '" . $memberDetails[3] . "' OR memberID = '" . $memberDetails[0] . "') AND memberPasscode = '" . $pass_word . "'");
						if ($memberrLoginQry) {
							if (mysqli_num_rows($memberrLoginQry) > 0) {
								$loginDate = date("Y-m-d H:i:s");
								$_SESSION["memberrIddd"] = $memberDetails[0];
								$_SESSION["memberrName"] = $memberDetails[1];
								$_SESSION["memberrMobile"] = $memberDetails[3];
								$_SESSION["memberrEmail"] = $memberDetails[4];
								$_SESSION["memberrPhoto"] = $memberDetails[5];

								$_SESSION["userType"] = 'Member';
								mysqli_query($conn, "INSERT INTO `member_logins`(`memberID`, `loginDateTime`) VALUES ('" . $_SESSION["memberrIddd"] . "', '" . $loginDate . "')") or die(mysqli_error($conn));
								mysqli_query($conn, "UPDATE members SET memberLoginStatus = '1' WHERE memberID = '" . $_SESSION["memberrIddd"] . "'") or die(mysqli_error($conn));
								$data = json_encode(array('Status' => true, 'Message' => 'memberLoginSuccessful'));
							} else {
								$errorMsg = "Sorry, password is incorrect. Try again!";;
								$errorList[] = $errorMsg;
								$data = json_encode(array('Status' => false, 'Message' => $errorList));
...
function headerImageReport($conn)
{
	global $ereyFure;
	global $errorList;
	$data = array();
			...
			if (substr($file, 0, 5) == "photo") {
				if (isset($_FILES[$file])) {
					...
					if (count($errorList) > 0) {
						...
						$fileNewName =  "Header_Image_". $valID . "_" . $p . "." . $fileInfo[2];
	...
	if ($commandType == "insert_command") {
		...
		if ($tn == 'members') {
			...
			if (mysqli_num_rows($getMaxID) > 0) {
				$maxID = mysqli_fetch_array($getMaxID);
				$curMaxID = $maxID[0] + 1;
				$generatedID = sprintf('%03d', $curMaxID);
			} else {
				$curMaxID = 1;
				$generatedID = sprintf('%03d', $curMaxID);
			}
			...
			if (count($errorList) <= 0) {
				...
				if ($regQry) {
					if ($tn == "members") {
						...
						$getMemberrMenuss = mysqli_query($conn, "SELECT * FROM member_menus");
						if ($getMemberrMenuss) {
							if (mysqli_num_rows($getMemberrMenuss) > 0) {
								while ($member_menu_rs = mysqli_fetch_array($getMemberrMenuss)) {
									mysqli_query($conn, "INSERT INTO member_privileges(memberID, memberMenuID, mainMenuMode) VALUES ('" . mysqli_real_escape_string($conn, $generatedID) . "', '" . $member_menu_rs[0] . "', '" . $member_menu_rs[4] . "')");
								}
							} else {
							}
						} else {
					...
					if ($tn == "users") {
						...
						if ($get_db_tables) {
							if (mysqli_num_rows($get_db_tables) > 0) {
								$notAllowedTables = array("main_menus", "sub_menus", "user_main_menus", "user_sub_menus", "user_role_privileges", "user_logins", "payment_gateways", "system_configurations");
								while ($role_rs = mysqli_fetch_array($get_db_tables)) {
									if (in_array($role_rs[0], $notAllowedTables)) {
										continue;
									} else {
										$saveRole = mysqli_query($conn, "INSERT INTO user_role_privileges(userID, roleName, insertRole, updateRole, deleteRole) VALUES ('" . mysqli_real_escape_string($conn, $generatedID) . "', '" . mysqli_real_escape_string($conn, $role_rs[0]) . "', '0', '0', '0')") or die(mysqli_error($conn));
...
// =========================== Fill Active Member Only ========================
function fill_patients_only($conn)
{

	$active_members_data = array();
	$data = array();

	$activeMemberIdd = mysqli_query($conn, "SELECT * FROM patients  ORDER BY patient_name ASC") or die(mysqli_error($conn));
	if ($activeMemberIdd) {
		$num_rows = mysqli_num_rows($activeMemberIdd);
		if ($num_rows > 0) {
			while ($row = mysqli_fetch_array($activeMemberIdd)) {
...
function fill_doctor_department($conn)
{

	$department_data = array();
	$data = array();

	$doctorDepartment = mysqli_query($conn, "SELECT * FROM departments") or die(mysqli_error($conn));
	if ($doctorDepartment) {
		$num_rows = mysqli_num_rows($doctorDepartment);
		if ($num_rows > 0) {
			while ($row = mysqli_fetch_array($doctorDepartment)) {
				$department_data[] = $row;
			}
			$data = json_encode(array('Status' => true, 'Message' => $department_data));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Department Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
...
function fill_diseases($conn)
{

	$disease_data = array();
	$data = array();

	$diseases = mysqli_query($conn, "SELECT * FROM diseases") or die(mysqli_error($conn));
	if ($diseases) {
		$num_rows = mysqli_num_rows($diseases);
		if ($num_rows > 0) {
			while ($row = mysqli_fetch_array($diseases)) {
				$disease_data[] = $row;
			}
			$data = json_encode(array('Status' => true, 'Message' => $disease_data));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Disease Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
...
function fill_disease_drugs($conn)
{
	extract($_POST);
	$parameterized_disease_drugs_data = array();
	$data = array();
	$parameterizedDiseaseDrug = mysqli_query($conn, "SELECT * FROM diseases WHERE disease_id = '" . mysqli_real_escape_string($conn, $objectID) . "'") or die(mysqli_error($conn));
	if ($parameterizedDiseaseDrug) {
		if (mysqli_num_rows($parameterizedDiseaseDrug) > 0) {
			$row = mysqli_fetch_array($parameterizedDiseaseDrug);
			$parameterized_disease_drugs_data[] = $row;
			$data = json_encode(array('Status' => true, 'Message' => $parameterized_disease_drugs_data));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Disease Drug Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
...
function fill_doctor_name($conn)
{
	extract($_POST);
	$parameterized_doctor_data = array();
	$data = array();
	$parameterizedDoctorName = mysqli_query($conn, "SELECT * FROM doctors WHERE doctor_department = '" . mysqli_real_escape_string($conn, $objectID) . "'") or die(mysqli_error($conn));
	if ($parameterizedDoctorName) {
		if (mysqli_num_rows($parameterizedDoctorName) > 0) {
			$row = mysqli_fetch_array($parameterizedDoctorName);
			$parameterized_doctor_data[] = $row;
			$data = json_encode(array('Status' => true, 'Message' => $parameterized_doctor_data));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Doctor Name Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
...
// ========================= Start  Fill Appointment Fee ============================

function fill_appointment_fee($conn)
{


	extract($_POST);
	$data = array();
	$parameterizedAppointmentFee = mysqli_query($conn, "SELECT appointment_fee FROM departments WHERE department_id = '" . mysqli_real_escape_string($conn, $objectID) . "'") or die(mysqli_error($conn));
	if ($parameterizedAppointmentFee) {
		if (mysqli_num_rows($parameterizedAppointmentFee) > 0) {
			$row = mysqli_fetch_array($parameterizedAppointmentFee);
			$parameterized_appointment_fee_data = $row[0];
			$data = json_encode(array('Status' => true, 'Message' => $parameterized_appointment_fee_data));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Appoinment Fee Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
...
// ========================= End  FIll Appoinment Fee ============================



// =================== Fill  Doctor Patients List =================================

function fill_doctor_patients_list($conn)
{
	$doctor_patients_list_data = array();
	$data = array();

	$doctor_patient_appointments = mysqli_query($conn, "SELECT appointment_view.rowNo,appointment_view.appointment_id,appointment_view.patient_name,appointment_view.doctor_name,appointment_view.appointment_number,appointment_view.Appointment_Status FROM appointment_view WHERE appointment_view.doctor_id = 'DOC1-2022' ORDER BY appointment_id ASC") or die(mysqli_error($conn));
	if ($doctor_patient_appointments) {
		$num_rows = mysqli_num_rows($doctor_patient_appointments);
		if ($num_rows > 0) {
			while ($row = mysqli_fetch_array($doctor_patient_appointments)) {
				$doctor_patients_list_data[] = $row;
			}
			$data = json_encode(array('Status' => true, 'Message' => $doctor_patients_list_data));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Patient Appointment Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
...
// ========================= End  Doctor Patients List ============================



function generate_appointment_number($conn)
{
	extract($_POST);
	$doctor_patients_list_data = array();
	$data = array();
	$startFrom = date("Y-m-d 00:00:00");
	$endTo = date("Y-m-d H:i:s");
	$getMaxID = mysqli_query($conn, "SELECT MAX(appointment_number) FROM appointments WHERE register_date BETWEEN '" . $startFrom . "' AND '" . $endTo . "' AND doctor_id = '" . mysqli_real_escape_string($conn, $objectID) . "'") or die(mysqli_error($conn));
	if (mysqli_num_rows($getMaxID) > 0) {
		$maxID = mysqli_fetch_array($getMaxID);
		$curMaxID = $maxID[0] + 1;
		// $generatedID = "APP".$curMaxID . "-" .date("Y");
	} else {
		$curMaxID = 1;
	...
	if ($getMaxID) {
		$num_rows = mysqli_num_rows($getMaxID);
		if ($num_rows > 0) {
			while ($row = mysqli_fetch_array($getMaxID)) {
				$curMaxID[] = $row;
			}
			$data = json_encode(array('Status' => true, 'Message' => $curMaxID));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Appointment Number Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
...
function fill_patients($conn)
{

	$patient_data = array();
	$data = array();

	$patients = mysqli_query($conn, "SELECT * FROM patients") or die(mysqli_error($conn));
	if ($patients) {
		$num_rows = mysqli_num_rows($patients);
		if ($num_rows > 0) {
			while ($row = mysqli_fetch_array($patients)) {
				$patient_data[] = $row;
			}
			$data = json_encode(array('Status' => true, 'Message' => $patient_data));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Department Not Found"));
		}
	} else {
				...
				<div class="tile-body">
					...
					<div class="row" style="margin-left: 0px; margin-right: 0px;">
						<div class="col-md-12">
									...
									<tbody class="allMembersCharge">
										...
										?>
											<tr>
												...
												<td align="center"><?php echo $result[0]; ?></td>
							...
							<script type="text/javascript">
								$('#chargeMemberContributionsTable').DataTable({
									"pageLength": 100
								});
							</script>
						</div>
			...
			<div class="tile">
				<div class="tile-body">
					...
					<div class="row">
						<div class="col-md-12" style="margin: 0px;">
							<center>
								<h5 style="font-family: 'Tw Cen MT'; color: maroon; font-weight: normal">No member(s) found</h5>
							</center>
						</div>
					</div>
					<br>
...
function charge_members_contribution($conn)
{
	...
	foreach ($selectedMembers as $memberrr) {
		...
		$chargeContributions = mysqli_query($conn, "INSERT INTO charge_pay_contributions VALUES ('', '" . $generatedID . "', '" . $memberr_id . "', '" . $not_appicable . "', '" . $contribution_yearss . "', '" . $contribution_months . "', '" . $total_contribution . "', '" . $paid_amount . "', '" . $charge_description . "', '" . $transactType . "', '" . $transactStatus . "', '" . $_SESSION['userIdd'] . "', '" . $chargeDatee . "', '-', '-')");
		$numOfChargedMembers++;
	}
...
// ================ Start of Members Change Password =======================
function member_change_password($conn)
{
	...
	if ($newPassChange != $confPassChange) {
		...
		$checkOldPass = mysqli_query($conn, "SELECT * FROM members WHERE memberID = '" . $_SESSION['memberrIddd'] . "' AND memberPasscode = '" . $oldPassChange . "'");
		if ($checkOldPass) {
			$oldPassNum = mysqli_num_rows($checkOldPass);
			if ($oldPassNum > 0) {
				$changePass = mysqli_query($conn, "UPDATE members SET memberPasscode = '" . $newPassChange . "' WHERE memberID = '" . $_SESSION['memberrIddd'] . "'");
				if ($changePass) {
					$data = json_encode(array('Status' => true, 'Message' => 'Password has been changed successfully'));
				} else {
					$errorMsg = mysqli_error($conn);
			...
			<div class="tile-body">
				...
				<div class="row" style="margin-left: 0px; margin-right: 0px;" id="employeesReportPrintArea">
					<div class="col-md-12">
						<div class="row">
							<div class="col-md-12" style="margin: 0px;">
								<img src="img/reportHeader.png" width="100%" height="200px" /><br>
								<center>
						...
						<div class="row">
							...
							<div class="col-md-12" style="background-image: url('img/reportBackground.png'); background-size : 90% 800px; background-repeat : repeat-y; background-position: center; -webkit-print-color-adjust: exact;">
								...
								?>
									...
									<div class="table-responsive">
										<table class="table table-bordered table-sm" style="font-size: 25px;">
											<thead>
												<tr class="bg-light">
			...
			<div class="tile-body">
				...
				<div class="row" style="margin-left: 0px; margin-right: 0px;" id="salaryPaymentsPrintArea">
					<div class="col-md-12">
						<div class="row">
							<div class="col-md-12" style="margin: 0px;">
								<img src="img/reportHeader.png" width="100%" height="200px" /><br><br>
								<center>
...
function patients_report($conn)
{

	extract($_POST);
	$patient_iddd = mysqli_real_escape_string($conn, $cmbPatientReportName);
	$str_datee = mysqli_real_escape_string($conn, substr($strDatePatientsReport, 0, 10) . " 00:00:00");
	$end_datee = mysqli_real_escape_string($conn, substr($endTooPatientsReport, 0, 10) . " 23:59:59");

	$qry = "";

	if ($patient_iddd == "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, 'SELECT patients.rowNo,patients.patient_id,patients.patient_name,CASE WHEN patients.patient_sex = 0 THEN "Male" WHEN patients.patient_sex = 1 THEN "Female" END as patient_sex,patients.patient_address,patients.patient_tell,patients.registered_by,patients.register_date FROM patients
		ORDER BY patient_name') or die(mysqli_error($conn));
	} else if ($patient_iddd == "" && $str_datee != " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT * FROM `patients` WHERE register_date BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND titlePosition <> 0 ORDER BY patient_name") or die(mysqli_error($conn));
	} else if ($patient_iddd != "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT * FROM `patients` WHERE patient_id = '" . $patient_iddd . "' AND titlePosition <> 0 ORDER BY empName") or die(mysqli_error($conn));
	} else {
		$qry = mysqli_query($conn, "SELECT * FROM `patients` WHERE patient_id = '" . $patient_iddd . "' AND registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND titlePosition <> 0 ORDER BY patient_name") or die(mysqli_error($conn));
	}
		...
		<div class="tile">
			<div class="tile-body">
				<div class="row" style="margin: 0px;">
					<div class="col-md-12 text-right">
						<br>
						<button class="btn btn-success btn-sm" id="printPatientsReport"> <i class="fa fa-print"></i> Print </button>
						<br><br>
					</div>
				</div>
				<div class="row" style="margin-left: 0px; margin-right: 0px;" id="patientsReportPrintArea">
					<div class="col-md-12">
						<div class="row">
							<div class="col-md-12" style="margin: 0px;">
								<img src="img/reportHeader.png" width="100%" height="200px" /><br>
								<center>
									<h2 style="color: #010132;">Patients Report</h2>
								</center>
							</div>
						</div>
						...
						<?php
						if ($patient_iddd != "") {
							$get_pat_name = mysqli_query($conn, "SELECT patient_name from patients WHERE patient_id = '" . $patient_iddd . "'") or mysqli_error($conn);
							$pat_rs = mysqli_fetch_array($get_pat_name);
							$patb_name = $mem_rs[0];
						?>
							<div class="row">
								<div class="col-md-8">
								...
								<div class="col-md-4 text-left" style="padding: 0px 20px 0px 0px;">
									<h5 class="text-left" style="font-size: 15px; font-weight: normal; padding-left: 15px;"><b>Report Date:</b> <?php echo date("d M Y H:i"); ?></h5>
									<h5 class="text-left" style="font-size: 15px; font-weight: normal; padding-left: 15px;"><b>Patient Name:</b> <?php echo $patb_name; ?></h5>
								</div>
							</div>
						<?php
						} else {
						...
						<div class="row">
							...
							<div class="col-md-12" style="background-image: url('img/reportBackground.png'); background-size : 90% 800px; background-repeat : repeat-y; background-position: center; -webkit-print-color-adjust: exact;">
								...
								<?php
									...
									$patient_sex = $result[3];
									$patient_address = $result[4];
									$patient_tell = $result[5];
									$registered_by = $result[6];
									$register_date = substr($result[7], 0, 16);
								...
								?>
									...
									<div class="row" style="margin: 0px;">
										<div class="col-md-4">
											<h5 style="border-bottom: 1px dashed black; font-size: 16px; padding: 8px 8px 8px 0px;">Patient ID</h5>
											<h5 style="font-size: 15px; font-weight: normal;"><?php echo $patient_id; ?></h5>
										</div>
										<div class="col-md-4">
											<h5 style="border-bottom: 1px dashed black; font-size: 16px; padding: 8px 8px 8px 0px;">Patient Name</h5>
											<h5 style="font-size: 15px; font-weight: normal;"><?php echo $patient_name; ?></h5>
										</div>
										<div class="col-md-4">
											<h5 style="border-bottom: 1px dashed black; font-size: 16px; padding: 8px 8px 8px 0px;">Sex</h5>
											<h5 style="font-size: 15px; font-weight: normal;"><?php echo $patient_sex; ?></h5>
										</div>
										<div class="col-md-4">
											<h5 style="border-bottom: 1px dashed black; font-size: 16px; padding: 8px 8px 8px 0px;">Address</h5>
											<h5 style="font-size: 15px; font-weight: normal;"><?php echo $patient_address; ?></h5>
										</div>
									</div>
									<div class="row" style="margin: 8px 0px 0px 0px;">
										<div class="col-md-4">
											<h5 style="border-bottom: 1px dashed black; font-size: 16px; padding: 8px 8px 8px 0px;">Phone</h5>
											<h5 style="font-size: 15px; font-weight: normal;"><?php echo $patient_tell; ?></h5>
										</div>
									...
									<div class="row" style="margin: 8px 0px 0px 0px;">
										...
										<div class="col-md-4">
											<h5 style="border-bottom: 1px dashed black; font-size: 16px; padding: 8px 8px 8px 0px;">Registered By</h5>
											<h5 style="font-size: 15px; font-weight: normal;"><?php echo $registered_by; ?></h5>
										</div>
										<div class="col-md-4">
											<h5 style="border-bottom: 1px dashed black; font-size: 16px; padding: 8px 8px 8px 0px;">Register Date</h5>
											<h5 style="font-size: 15px; font-weight: normal;"><?php echo substr($register_date, 0, 16); ?></h5>
										</div>
									</div>
									<br><br>
		...
		<div class="tile">
			<div class="tile-body">
				...
				<div class="row">
					<div class="col-md-12" style="margin: 0px;">
						<center>
							<h3 style="font-family: 'Tw Cen MT'; color: maroon; font-weight: normal">Patient(s) information not found</h3>
						</center>
					</div>
				</div>
				<br>
...
// ---------------- Print members report group ---------------------------
function members_report($conn)
{

	extract($_POST);
	$memberr_iddd = mysqli_real_escape_string($conn, $memberNameMembersReport);
	$str_datee = mysqli_real_escape_string($conn, substr($strDateMembersReport, 0, 10) . " 00:00:00");
	$end_datee = mysqli_real_escape_string($conn, substr($endTooMembersReport, 0, 10) . " 23:59:59");

	$qry = "";

	if ($memberr_iddd == "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberID, members.memberName, members.memberTelephone, members.memberEmail, contribution_types.contributionAmount, members.memberStatus, users.userName FROM members JOIN contribution_types ON (members.contributionTypeID = contribution_types.contributionTypeID) JOIN users ON (members.registeredBy = users.userID) ORDER BY members.memberName ASC") or die(mysqli_error($conn));
	} else if ($memberr_iddd == "" && $str_datee != " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT * FROM `employees` WHERE registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND titlePosition <> 0 ORDER BY empName") or die(mysqli_error($conn));
	} else if ($memberr_iddd != "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT * FROM `employees` WHERE empID = '" . $memberr_iddd . "' AND titlePosition <> 0 ORDER BY empName") or die(mysqli_error($conn));
	} else {
		$qry = mysqli_query($conn, "SELECT * FROM `employees` WHERE empID = '" . $memberr_iddd . "' AND registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND titlePosition <> 0 ORDER BY empName") or die(mysqli_error($conn));
	}
					...
					<div class="col-md-12">
						<div class="row">
							<div class="col-md-12" style="margin: 0px;">
								<img src="img/reportHeader.png" width="100%" height="200px" /><br>
								<center>
									<h2 style="color: #010132;">Members Report</h2>
								</center>
							</div>
						</div>
						...
						<?php
						if ($memberr_iddd != "") {
							$get_mem_name = mysqli_query($conn, "SELECT memberName from members WHERE memberID = '" . $memberr_iddd . "'") or mysqli_error($conn);
							$mem_rs = mysqli_fetch_array($get_mem_name);
							$memb_name = $mem_rs[0];
						?>
							<div class="row">
								<div class="col-md-8">
								...
								<div class="col-md-4 text-left" style="padding: 0px 20px 0px 0px;">
									<h5 class="text-left" style="font-size: 15px; font-weight: normal; padding-left: 15px;"><b>Report Date:</b> <?php echo date("d M Y H:i"); ?></h5>
									<h5 class="text-left" style="font-size: 15px; font-weight: normal; padding-left: 15px;"><b>Member Name:</b> <?php echo $memb_name; ?></h5>
								</div>
							</div>
						<?php
						} else {
										...
										<table class="table table-bordered table-sm" style="font-size: 25px;">
											<thead>
												<tr class="bg-light">
													...
													<th style="border-bottom: 1px solid black;">
														<center>Monthly Paid Amount ($)</center>
													</th>
													<th style="border-bottom: 1px solid black;">
														<center>Status</center>
													</th>
											...
											<tbody>
												...
												?>
													<tr>
														...
														<td align="center">
															<?php if ($result[5] == "1") {
																echo "<span class='badge badge-success col-12'><i class='fa fa-check-circle'></i> Active</span>";
															} else if ($result[5] == "0") {
																echo "<span class='badge badge-danger col-12'><i class='fa fa-times-circle'></i> InActive</span>";
															} ?>
														</td>
														<td align="left"> <?php echo $result[6]; ?></td>
...
// ---------------- Print member charges report group ---------------------
function member_charges_report($conn)
{

	extract($_POST);

	$member_idd = mysqli_real_escape_string($conn, $memberNameMemberChargesReport);
	$contribution_charge_year = mysqli_real_escape_string($conn, $contributionYearMemberChargesReport);
	$str_datee = mysqli_real_escape_string($conn, substr($strDateMemberChargesReport, 0, 10) . " 00:00:00");
	$end_datee = mysqli_real_escape_string($conn, substr($endTooMemberChargesReport, 0, 10) . " 23:59:59");

	$qry = "";

	if ($member_idd == "" && $contribution_charge_year == "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.transactionType = '0' AND charge_pay_contributions.transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	} else if ($member_idd == "" && $contribution_charge_year == "" && $str_datee != " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND charge_pay_contributions.transactionType = '0' AND charge_pay_contributions.transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	} else if ($member_idd == "" && $contribution_charge_year != "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.contributionOf = '" . $contribution_charge_year . "' AND charge_pay_contributions.transactionType = '0' AND charge_pay_contributions.transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	} else if ($member_idd == "" && $contribution_charge_year != "" && $str_datee != " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.contributionOf = '" . $contribution_charge_year . "' AND charge_pay_contributions.registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND charge_pay_contributions.transactionType = '0' AND charge_pay_contributions.transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	} else if ($member_idd != "" && $contribution_charge_year == "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.memberID = '" . $member_idd . "' AND charge_pay_contributions.transactionType = '0' AND charge_pay_contributions.transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	} else if ($member_idd != "" && $contribution_charge_year == "" && $str_datee != " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.memberID = '" . $member_idd . "' AND charge_pay_contributions.registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND charge_pay_contributions.transactionType = '0' AND charge_pay_contributions.transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	} else if ($member_idd != "" && $contribution_charge_year != "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.memberID = '" . $member_idd . "' AND charge_pay_contributions.contributionOf = '" . $contribution_charge_year . "' AND charge_pay_contributions.transactionType = '0' AND charge_pay_contributions.transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	} else {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.memberID = '" . $member_idd . "' AND charge_pay_contributions.contributionOf = '" . $contribution_charge_year . "' AND charge_pay_contributions.registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND charge_pay_contributions.transactionType = '0' AND charge_pay_contributions.transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	}
		...
		<div class="tile">
			<div class="tile-body">
				<div class="row" style="margin: 0px;">
					<div class="col-md-12 text-right">
						<br>
						<button class="btn btn-success btn-sm" id="printMemberChargesReport"> <i class="fa fa-print"></i> Print </button>
						<br><br>
					</div>
				</div>
				<div class="row" style="margin-left: 0px; margin-right: 0px;" id="memberChargesPrintArea">
					<div class="col-md-12">
						<div class="row">
							<div class="col-md-12" style="margin: 0px;">
								<img src="img/reportHeader.png" width="100%" height="200px" /> <br><br>
								<center>
									<h2 style="font-family: 'Arial'; color: #010132;">Member Charges Report</h2>
								</center>
							</div>
						</div>
						...
						<?php
						if ($member_idd != "" && $contribution_charge_year == "") {
							$get_member_name = mysqli_query($conn, "SELECT memberName from members WHERE memberID = '" . $member_idd . "'") or mysqli_error($conn);
						...
						?>
							<div class="row">
								...
								<div class="col-md-4 text-left" style="padding: 0px 20px 0px 0px;">
									<h5 class="text-left" style="font-family: 'century gothic'; font-weight: normal; padding-left: 15px;"><b>Report Date:</b> <?php echo date("d M Y H:i"); ?></h5>
									<h5 class="text-left" style="font-family: 'century gothic'; font-weight: normal; padding-left: 15px;"><b>Member Name:</b> <?php echo $memb_name; ?></h5>
								</div>
							</div>
						<?php
						} else if ($member_idd == "" && $contribution_charge_year != "") {
						?>
							<div class="row">
								<div class="col-md-8">
									&nbsp;
								...
								<div class="col-md-4 text-left" style="padding: 0px 20px 0px 0px;">
									<h5 class="text-left" style="font-family: 'century gothic'; font-weight: normal; padding-left: 15px;"><b>Report Date:</b> <?php echo date("d M Y H:i"); ?></h5>
									<h5 class="text-left" style="font-family: 'century gothic'; font-weight: normal; padding-left: 15px;"><b>Contribution Year:</b> <?php echo $contribution_charge_year; ?></h5>
								</div>
							</div>
						<?php
						} else {
						?>
							<div class="row">
								...
								<div class="col-md-4 text-left" style="padding: 0px 20px 0px 0px;">
									<h5 class="text-left" style="font-family: 'century gothic'; font-weight: normal; padding-left: 15px;"><b>Report Date:</b> <?php echo date("d M Y H:i"); ?></h5>
									<h5 class="text-left" style="font-family: 'century gothic'; font-weight: normal; padding-left: 15px;"><b>Report Type: </b> General (All)</h5>
								</div>
							</div>
						<?php
										...
										<tbody>
											...
											?>
												<tr>
													<td align="center"> <?php echo $i; ?></td>
													<td align="left"> <?php echo $result[0]; ?></td>
													<td align="center"> <?php echo $result[1]; ?></td>
													<td align="center"> <?php echo $result[2]; ?></td>
											...
											<?php
												$totalMemberChargedAmount += $result[3];
												$i++;
											}
											?>

											<tr>
												<td align="right" colspan="6"><b>Total Charged Amount:</b></td>
												<td align="center"> <b><?php echo "$ " . number_format($totalMemberChargedAmount, 2); ?></b></td>
											</tr>
		...
		<div class="tile">
			<div class="tile-body">
				...
				<div class="row">
					<div class="col-md-12" style="margin: 0px;">
						<center>
							<h5 style="font-family: 'Tw Cen MT'; color: maroon; font-weight: normal">No member charges found</h5>
						</center>
					</div>
				</div>
				<br>
...
// ---------------- Print member payments report group --------------------
function member_payments_report($conn)
{

	extract($_POST);

	$memberr_idd = mysqli_real_escape_string($conn, $memberInfoMemberPaymentsReport);
	$str_datee = mysqli_real_escape_string($conn, substr($strDateMemberPaymentsReport, 0, 10) . " 00:00:00");
	$end_datee = mysqli_real_escape_string($conn, substr($endTooMemberPaymentsReport, 0, 10) . " 23:59:59");

	$qry = "";

	if ($memberr_idd == "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberID, members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, payment_gateways.gatewayType, payment_gateways.gatewayName, account_numbers.accountNoName, account_numbers.accountNumber, charge_pay_contributions.paidAmount, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) LEFT JOIN account_numbers ON (charge_pay_contributions.accountNoID = account_numbers.accountNoID) LEFT JOIN payment_gateways ON (account_numbers.gatewayID = payment_gateways.gatewayID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate ASC") or die(mysqli_error($conn));
	} else if ($memberr_idd == "" && $str_datee != " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberID, members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, payment_gateways.gatewayType, payment_gateways.gatewayName, account_numbers.accountNoName, account_numbers.accountNumber, charge_pay_contributions.paidAmount, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) LEFT JOIN account_numbers ON (charge_pay_contributions.accountNoID = account_numbers.accountNoID) LEFT JOIN payment_gateways ON (account_numbers.gatewayID = payment_gateways.gatewayID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate ASC") or die(mysqli_error($conn));
	} else if ($memberr_idd != "" && $str_datee == " 00:00:00") {
		...
		$qry = mysqli_query($conn, "SELECT members.memberID, members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, payment_gateways.gatewayType, payment_gateways.gatewayName, account_numbers.accountNoName, account_numbers.accountNumber, charge_pay_contributions.paidAmount, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) LEFT JOIN account_numbers ON (charge_pay_contributions.accountNoID = account_numbers.accountNoID) LEFT JOIN payment_gateways ON (account_numbers.gatewayID = payment_gateways.gatewayID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.memberID = '" . $memberr_idd . "' AND charge_pay_contributions.registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate ASC") or die(mysqli_error($conn));
	}
					...
					<div class="col-md-12">
						...
						?>
							<div class="row">
								...
								<div class="col-md-4 text-left" style="padding: 0px 20px 0px 0px;">
									<h6 class="text-left" style="font-size: 15px; font-weight: normal; padding-left: 15px;"><b>Report Date:</b> <?php echo date("d M Y H:i"); ?></h6>
									<h6 class="text-left" style="font-size: 15px; font-weight: normal; padding-left: 15px;"><b>Report Type: </b> General (All)</h6>
								</div>
							</div>
						<?php
						...
						<div class="row">
							...
							?>
								<div class="col-md-4 text-left">
									<h6 class="text-left" style="font-size: 15px; font-weight: normal;"><b>Start Date (From):</b> <?php echo substr($str_datee, 0, 10); ?></h6>
								</div>
								<div class="col-md-3 text-left">
									<h6 class="text-left" style="font-size: 15px; font-weight: normal;"><b>End Date (To):</b> <?php echo substr($end_datee, 0, 10); ?></h6>
								</div>
							<?php
							}
							?>
										...
										<tbody>
											...
											<tr>
												...
												<td align="center" colspan="2">
													<h6><b>Remaining Balance</b></h6>
													<h6 style="font-weight: normal;"><?php echo "<b><span class='text-danger'> $ " . number_format(($totalContributionChargedAmount - $totalContributionPaidAmount), 2), "</span></b>"; ?></h6>
												</td>
											</tr>
...
function member_charge_history($conn)
{

	extract($_POST);

	$member_idd = mysqli_real_escape_string($conn, $_SESSION['memberrIddd']);
	$str_datee = mysqli_real_escape_string($conn, substr($strDateContributionChargeHistory, 0, 10) . " 00:00:00");
	$end_datee = mysqli_real_escape_string($conn, substr($endTooContributionChargeHistory, 0, 10) . " 23:59:59");

	$qry = "";

	if ($member_idd != "" && $str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID AND charge_pay_contributions.memberID = '" . $member_idd . "' AND charge_pay_contributions.transactionType = '0') JOIN users ON (charge_pay_contributions.registeredBy = users.userID) ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	} else if ($member_idd != "" && $str_datee != " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID AND charge_pay_contributions.memberID = '" . $member_idd . "' AND charge_pay_contributions.registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND charge_pay_contributions.transactionType = '0') JOIN users ON (charge_pay_contributions.registeredBy = users.userID) ORDER BY charge_pay_contributions.registerDate DESC") or die(mysqli_error($conn));
	}
			...
			<div class="tile-body">
				...
				<div class="row" style="margin-left: 0px; margin-right: 0px;" id="memberChargeHistoryPrintArea">
					<div class="col-md-12">
						...
						$get_memberr_name = mysqli_query($conn, "SELECT memberName from members WHERE memberID = '" . $member_idd . "'") or mysqli_error($conn);
...
// ---------------- Print member payment history --------------------
function member_payment_history($conn)
{

	extract($_POST);

	$memberr_idd = mysqli_real_escape_string($conn, $_SESSION['memberrIddd']);
	$str_datee = mysqli_real_escape_string($conn, substr($strDateMemberPaymentHistory, 0, 10) . " 00:00:00");
	$end_datee = mysqli_real_escape_string($conn, substr($endTooMemberPaymentHistory, 0, 10) . " 23:59:59");

	$qry = "";

	if ($str_datee == " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberID, members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, payment_gateways.gatewayType, payment_gateways.gatewayName, account_numbers.accountNoName, account_numbers.accountNumber, charge_pay_contributions.paidAmount, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) LEFT JOIN account_numbers ON (charge_pay_contributions.accountNoID = account_numbers.accountNoID) LEFT JOIN payment_gateways ON (account_numbers.gatewayID = payment_gateways.gatewayID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.memberID = '" . $memberr_idd . "' AND transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate ASC") or die(mysqli_error($conn));
	} else if ($str_datee != " 00:00:00") {
		$qry = mysqli_query($conn, "SELECT members.memberID, members.memberName, charge_pay_contributions.contributionOf, charge_pay_contributions.chargedMonths, payment_gateways.gatewayType, payment_gateways.gatewayName, account_numbers.accountNoName, account_numbers.accountNumber, charge_pay_contributions.paidAmount, charge_pay_contributions.chargedAmount, charge_pay_contributions.registerDate, users.userName FROM charge_pay_contributions JOIN members ON (members.memberID = charge_pay_contributions.memberID) LEFT JOIN account_numbers ON (charge_pay_contributions.accountNoID = account_numbers.accountNoID) LEFT JOIN payment_gateways ON (account_numbers.gatewayID = payment_gateways.gatewayID) JOIN users ON (charge_pay_contributions.registeredBy = users.userID) WHERE charge_pay_contributions.memberID = '" . $memberr_idd . "' AND charge_pay_contributions.registerDate BETWEEN '" . $str_datee . "' AND '" . $end_datee . "' AND transactionStatus = '1' ORDER BY charge_pay_contributions.registerDate ASC") or die(mysqli_error($conn));
	}
